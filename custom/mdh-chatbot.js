...
if (!data.authenticated) {return {id: null,name: "guest",email: ""};}

      if (

data.bookstack_user_id === undefined ||data.bookstack_user_id === null) {return {id: null,name: "guest",email: ""};}
...
try {const bookStackUser =await getBookStackUser();
...
} catch (error) {chatbotWelcome.textContent ="Hello! How can I help you today?";
...
const bookStackUser =await getBookStackUser();

const userName =bookStackUser?.name || "guest";

const userId =bookStackUser?.id || null;

const langflowSessionId =userId? getLangflowSessionId(userName, userId): "guest";

const liteLLMUserId =userId? getLiteLLMUserId(userId): "guest";/** Display details in:
...
tweaks: {"CustomComponent-otOXa": {bookstack_user_id: userId || "guest",bookstack_user_name: userName || "guest",litellm_user: liteLLMUserId || "guest",langflow_session_id: langflowSessionId || "guest"}},
